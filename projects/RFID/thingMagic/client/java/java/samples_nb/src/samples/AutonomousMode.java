...
public class AutonomousMode 
{
    ...
    static void usage()
    {
        System.out.printf("Usage: Please provide valid arguments, such as:\n"
                + "  (URI: 'tmr:///COM1 --ant 1,2 --enable/disable' " 
                + "or 'tmr:///dev/ttyS0 --ant 1,2 --enable/disable')\n\n");
        System.exit(1);
    }
        ...
        try
        {
            ...
            for ( ; nextarg < argv.length; nextarg++)
            {
                ...
                else if(arg.equalsIgnoreCase("--enable"))
                {
                    ...
                    {
                        System.out.println("Duplicate argument: --enable specified more than once");
                        usage();
                    }
                    System.out.println("autonomousMode = \"true\";");
                    autonomousMode = "true";
                }
                else if(arg.equalsIgnoreCase("--disable"))
                {
                    if(autonomousMode != null)
                    {
                        System.out.println("Duplicate argument: --disable specified more than once");
                        usage();
                    }
                    autonomousMode = "false";
                    System.out.println("autonomousMode = \"false\";");
                }
                else
                {
                    ...
                    || model.equalsIgnoreCase("M6e Micro USBPro"))
            {
                ...
                if(!Boolean.valueOf(autonomousMode))
                {
                    r.paramSet("/reader/userConfig", new SerialReader.UserConfigOp(SerialReader.SetUserProfileOption.CLEAR));
                    System.out.println("User profile set option: reset autonomous mode operation ");
                }
                else
                {
    ...
    static class ReaderStatsListener implements StatsListener
    {
        ...
        {
            System.out.println("Temparature "+readerStats.temperature);
        }
    }
