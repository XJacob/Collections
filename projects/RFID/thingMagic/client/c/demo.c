...
void setcmd(char* name, char *value);
...
void getoneparam(const char *paramName)
{
  TMR_Status ret;
  TMR_Param param;
 ...
 * Iso18k6b.Lock:100
 */
int parseLockAction(const char *arg, TMR_TagLockAction *la)
{
...
//  printf("set parameter %s:%s\n",name,value);
  ...
  else if (TMR_PARAM_GEN2_Q == param)
  {
	...
	&& initialQ <= UINT8_MAX)
    {
      ...
      printf("type is %d, inital Q is %d\n",TMR_SR_GEN2_Q_STATIC,initialQ);
    }
    else
    {
